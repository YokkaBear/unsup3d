...
class Unsup3D():
    def __init__(self, cfgs):
        ...
        self.netD = networks.EDDeconv(cin=3, cout=1, nf=64, zdim=256, activation=None)  # depth map
        self.netA = networks.EDDeconv(cin=3, cout=3, nf=64, zdim=256)  # albedo image
        self.netL = networks.Encoder(cin=3, cout=4, nf=32)  # light direction
        self.netV = networks.Encoder(cin=3, cout=6, nf=32)  # viewpoint
        self.netC = networks.ConfNet(cin=3, cout=2, nf=64, zdim=128)  # confidence map
        self.network_names = [k for k in vars(self) if 'net' in k]
        self.make_optimizer = lambda model: torch.optim.Adam(
            filter(lambda p: p.requires_grad, model.parameters()),  # 对param列表用一个filter过滤，返回的还是一个参数惰性列表
            lr=self.lr, betas=(0.9, 0.999), weight_decay=5e-4)  # 以model作为输入，返回一个参数设定好的Adam优化器
    ...
    def forward(self, input):
        ...
        # print(self.canon_normal.shape)  # + db1 torch.Size([32, 256, 256, 3])
        # print(self.canon_light_d.view(-1,1,1,3).shape)  # + db2 torch.Size([7200, 1, 1, 3])
        self.canon_diffuse_shading = (self.canon_normal * self.canon_light_d.view(-1,1,1,3)).sum(3).clamp(min=0).unsqueeze(1)  # 这一行维度出了问题
        ...
        if self.load_gt_depth:
            ...
            self.sie_map_masked = utils.compute_sc_inv_err(self.recon_depth[:b].log(), self.depth_gt[:b].log(), mask=mask)  # compute diff between recon_depth and depth_gt (me)
            self.acc_sie_masked = (self.sie_map_masked.view(b,-1).sum(1) / mask.view(b,-1).sum(1))**0.5
            self.norm_err_map_masked = utils.compute_angular_distance(self.recon_normal[:b], self.normal_gt[:b], mask=mask)  # compute diff between recon_normal and normal_gt (me)
    ...
    def visualize(self, logger, total_iter, max_bs=25):
        ...
        if self.load_gt_depth:
            ...
            sie_map_masked = self.sie_map_masked[:b0].detach().unsqueeze(1).cpu() *1000  # get SIDE
            norm_err_map_masked = self.norm_err_map_masked[:b0].detach().unsqueeze(1).cpu() /100  # get MAD
    ...
    def save_results(self, save_dir):
        ...
        utils.save_images(save_dir, input_im_symline, suffix='input_image_symline', sep_folder=sep_folder)  # symline: 对称轴
