...
class Trainer():
    ...
    def test(self):
        ...
        self.model.save_scores(score_path)  # 只有load_gt设置成true才会生成eval_score.txt
    ...
    def train(self):
        ...
        model_name_local = self.model.model_name  # gaidong +
        # + 添加并行训练模块 开始 （没必要，已弃用）
        # if torch.cuda.device_count() > 1:
        #     # self.model = torch.nn.DataParallel(self.model)
        #     self.model = torch.nn.DataParallel(self.model, [0,1,2])  # multi-GPUs pass in gpu_ids=[0,1,2] directly
        # # + 结束

        ## resume from checkpoint 其中load_checkpoint函数保证了加载的模型是latest的模型
        if self.resume:
        ...
        if self.use_logger:
            ...
            self.viz_input = self.val_loader.__iter__().__next__()  # 用val中的数据进行的模型评价，此处为val中数据的loader

        ## run epochs
        # print(f"{self.model.model_name}: optimizing to {self.num_epochs} epochs")  # gaidong -
        print(f"{model_name_local}: optimizing to {self.num_epochs} epochs")  # gaidong +
        for epoch in range(start_epoch, self.num_epochs):
            self.current_epoch = epoch
            metrics = self.run_epoch(self.train_loader, epoch)  # 训练过程中，训练集train set将会被用到
            self.metrics_trace.append("train", metrics)

            with torch.no_grad():
                metrics = self.run_epoch(self.val_loader, epoch, is_validation=True)  # 同时验证集val set也会被用到
    ...
    def run_epoch(self, loader, epoch=0, is_validation=False, is_test=False):
        ...
        for iter, input in enumerate(loader):
            ...
            if iter % 100 == 0:  # gaidong: 每500次打印一次命令行
            ...
            if self.use_logger and is_train:
                ...
                if total_iter % self.log_freq == 0:
                    self.model.forward(self.viz_input)  # forward进去的是val_loader，也就是用的是val中的数据计算的各项评价指标及可视化
